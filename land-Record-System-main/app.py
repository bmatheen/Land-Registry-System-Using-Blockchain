...
def get_record():
    uid = request.args.get('uid')  # Get UID from the URL parameter
    for block in blockchain:
        if block.uid == uid:
            return render_template('record.html', record=block)  # Send record if found
    return render_template('record.html', record=None)  # Send None if UID not found
